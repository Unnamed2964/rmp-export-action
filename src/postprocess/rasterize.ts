...
function withWhiteBackground(svg: string): string {
  if (/<rect[^>]*id="rmp-export-bg"/.test(svg)) return svg;
  return svg.replace(
    /(<svg[^>]*>)/,
    '$1<rect id="rmp-export-bg" x="-100000" y="-100000" width="200000" height="200000" fill="#ffffff"/>',
  );
}

export async function rasterizeSvg(options: RasterizeOptions): Promise<void> {
