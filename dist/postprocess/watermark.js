...
const RMP_INFO_RE = /(<g\b[^>]*\btransform=")translate\([^"]*\)("[^>]*\bid="rmp_info"[^>]*>)/;
const RMP_INFO_TRANSLATE_RE = /\btransform="translate\(([^)]+)\)"[^>]*\bid="rmp_info"|\bid="rmp_info"[^>]*\btransform="translate\(([^)]+)\)"/;
const RMP_INFO_OPEN_RE = /<g\b[^>]*\bid="rmp_info"[^>]*>/;
function clamp(value, min, max) {
    return Math.min(Math.max(value, min), max);
}
function parseTranslatePair(raw) {
    const [x, y] = raw.split(/[\s,]+/).map(Number);
    if (Number.isNaN(x) || Number.isNaN(y))
        return null;
    return { x, y };
}
export function parseRmpInfoTranslate(svg) {
    const match = svg.match(RMP_INFO_TRANSLATE_RE);
    if (!match)
        return null;
    return parseTranslatePair(match[1] ?? match[2] ?? "");
}
function extractRmpInfoElement(svg) {
...
export function measureRmpInfoBBox(svg) {
...
export function measureRmpInfoGeometry(svg) {
...
function bboxFromTranslate(translate, geometry) {
    return {
...
function computeTargetBBox(frame, anchor, inset, geometry) {
...
function computeTranslateFromInset(frame, config, geometry) {
    if (config.absolute) {
...
function isRectInFrame(bbox, frame) {
    return (bbox.x >= frame.x &&
        bbox.y >= frame.y &&
        bbox.x + bbox.width <= frame.x + frame.width &&
        bbox.y + bbox.height <= frame.y + frame.height);
}
function clampRectToFrame(bbox, frame) {
    return {
...
function writeRmpInfoTranslate(svgPath, svg, translate) {
    if (!RMP_INFO_RE.test(svg))
        return false;
    writeFileSync(svgPath, svg.replace(RMP_INFO_RE, `$1translate(${translate.x}, ${translate.y})$2`), "utf-8");
    return true;
}
export function applyWatermark(svgPath, frame, config, geometry) {
...
export function ensureWatermarkInFrame(svgPath, frame, geometry) {
    ...
    if (frame.width < bbox.width || frame.height < bbox.height) {
        console.warn(`watermark ensure: frame (${frame.width}x${frame.height}) smaller than watermark (${bbox.width}x${bbox.height}), leaving position unchanged`);
        return;
    }
    ...
    console.warn(`watermark ensure: repositioned (${translate.x}, ${translate.y}) -> (${nextTranslate.x}, ${nextTranslate.y}) to fit viewBox`);
}
