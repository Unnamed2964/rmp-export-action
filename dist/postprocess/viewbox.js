...
export function parseViewBoxFromFile(svgPath) {
...
export function applyCropToSvg(svgPath, crop) {
    ...
    if (/<svg\b[^>]*\bviewBox="[^"]*"/.test(svg)) {
        svg = svg.replace(/(<svg\b[^>]*\b)viewBox="[^"]*"/, `$1viewBox="${viewBox}"`);
    }
    else {
        svg = svg.replace(/<svg\b/, `<svg viewBox="${viewBox}"`);
    }
    if (/<svg\b[^>]*\bwidth="[^"]*"/.test(svg)) {
        svg = svg.replace(/(<svg\b[^>]*\b)width="[^"]*"/, `$1width="${crop.width}"`);
    }
    if (/<svg\b[^>]*\bheight="[^"]*"/.test(svg)) {
        svg = svg.replace(/(<svg\b[^>]*\b)height="[^"]*"/, `$1height="${crop.height}"`);
    }
    writeFileSync(svgPath, svg, "utf-8");
    if (before) {
        console.log(`crop: viewBox ${before.x} ${before.y} ${before.width} ${before.height}`);
