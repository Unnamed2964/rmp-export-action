...
The action clones a specified RMP version, starts it in a browser, drives the export UI with Playwright on your behalf, post-processes SVG (crop, watermark repositioning, custom scripts), and generates WebP.
...
6. When the workflow runs successfully, open **Releases** on the repository (in the right sidebar on the repository home page) to view the new version. You can download SVG and WebP from the release attachments, and see the exported WebP inline in the release notes.
...
defaults:
  ...
  scale: 2.0
  # whiteBackground: When true, WebP uses a white background; otherwise transparent
